...
int main(void) {
  ...
  result = allocate_coeffs(MAXPOLY, &delay, &wet, &dry, &Az, &El, &U, &V, &W);
  ...
  result = runcalc2_c(60255.68715277778, 60255.6870, &npoly, delay[0], MAXPOLY);
  if (result!=0) {
    printf("Error (%d) running calc!", result);
  } else {
    printf("Got %d polynomials\n", npoly);
  
    for (int i=0; i<npoly; i++) {
      for (int j=0; j<CALC_POLY_ORDER+1; j++) {
	printf("%.6g ", delay[i][j]);
      }
      printf("\n");
    }
    ...
    //free_coeffs(MAXPOLY, delay, wet, dry, Az, El, U, V, W);
  }
